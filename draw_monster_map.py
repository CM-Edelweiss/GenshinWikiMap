...
def draw_monster_map(monster: Monster):
    ...
    for tap in taps:
        if tap:
            ...
            img.draw_rounded_rectangle(
                (now_tap_width, 142, now_tap_width + type_length + 10, 191),
                10,
                "#ff6f30",
            )
            img.text(
                tap,
                (now_tap_width, now_tap_width + type_length + 10),
                (140, 189),
                tap_font,
                "white",
                "center",
    ...
    if res := monster_detail.resistance:
        # 火
        res_str = (
            f"{int(res.fireSubHurt * 100)}%"
            if monster.name not in ["火史莱姆", "大型火史莱姆", "火飘浮灵", "无相之火"]
            else "免疫"
        )
        img.text(res_str, (677, 741), 790, description_font, "#252525")
        # 水
        res_str = (
            f"{int(res.waterSubHurt * 100)}%"
            if monster.name
            not in ["水史莱姆", "大型水史莱姆", "水飘浮灵", "无相之水", "纯水精灵"]
            else "免疫"
        )
        img.text(res_str, (773, 837), 790, description_font, "#252525")
        # 风
        res_str = (
            f"{int(res.windSubHurt * 100)}%"
            if monster.name
            not in [
                "风史莱姆",
                "大型风史莱姆",
        ...
        # 草
        res_str = (
            f"{int(res.grassSubHurt * 100)}%"
            if monster.name not in ["草史莱姆", "大型草史莱姆", "草飘浮灵", "无相之草"]
            else "免疫"
        )
        img.text(res_str, (1062, 1126), 790, description_font, "#252525")
        # 冰
        res_str = (
            f"{int(res.iceSubHurt * 100)}%"
            if monster.name
            not in [
                "冰史莱姆",
                "大型冰史莱姆",
        ...
        # 岩
        res_str = (
            f"{int(res.rockSubHurt * 100)}%"
            if monster.name not in ["岩史莱姆", "大型岩史莱姆", "岩飘浮灵", "无相之岩"]
            else "免疫"
        )
        img.text(res_str, (1253, 1317), 790, description_font, "#252525")
        # 物理
        img.text(
            f"{int(res.physicalSubHurt * 100)}%",
            (1349, 1413),
            790,
            description_font,
            "#252525",
    ...
    if monster_detail.reward:
        ...
        materials = [
            ...
            if material["name"] not in ignore_material
        ...
        if materials:
            ...
            for i, material in enumerate(materials):
                ...
                material_icon = download_from_ambr(
                    MATERIALS / f'{material["icon"]}.png'
                )
                ...
                if charas:
                    ...
                    for j, chara in enumerate(charas):
                        img.paste(
                            CHARACTER_MAP_RESOURCES / "圆框橙.png",
                            (258 + j * 148, 1172 + i * 181),
                        )
                        chara_icon = PMImage(
                            RESOURCES / "avatar_new_m" / f"{chara}.png"
                        )
