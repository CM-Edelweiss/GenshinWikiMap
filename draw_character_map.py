        ...
        for name, value in promote_data.items():
            ...
            img.draw_rounded_rectangle(
                (
                    this_width,
                    now_height,
                    this_width + promote_text_size[0] + 10,
                    now_height + 36,
                ),
                5,
                "#ff6f30",
...
def draw_character_map(chara: Character):
    ...
    if chara.beta:
        img.text(
            "测试角色", 950 if chara.rank == 5 else 900, 64, main_text_font, "#252525"
        )
    # 角色名称标题
    img.text(chara.title_name, 573, 83, title_font, "#252525")
    # ---------------立绘---------------
    offset = load_json(DATA / "角色立绘偏移已用.json").get(chara.id, [0, 0])
    gacha_img = download_from_ambr(
        GACHA_IMG / f'UI_Gacha_AvatarImg_{chara.icon.split("_")[-1]}.png'
    )
    img.paste(CHARACTER_MAP_RESOURCES / "立绘下花纹.png", (35, 40))
    gacha_img = gacha_img.crop(
        (
    ...
    clearance = int(
        (800 - (hp_length + atk_length + def_length + ext_length + bir_length)) / 4
    )
    now_width = 660
    # 生命值
    ...
    img.text(
        str(chara.attack),
        (now_width, now_width + atk_length),
        308,
        prop_value_font,
        "#252525",
        "center",
    ...
    img.text(
        str(chara.defense),
        (now_width, now_width + def_length),
        308,
        prop_value_font,
        "#252525",
        "center",
    ...
    img.text(
        extra_prop_value_str,
        (now_width, now_width + ext_length),
        308,
        prop_value_font,
        "#252525",
        "center",
    ...
    img.text(
        chara.birthday if chara.birthday != "0/0" else "-",
        (now_width, now_width + bir_length),
        308,
        prop_value_font,
        "#252525",
        "center",
    ...
    for material in materials:
        ...
        img.draw_rounded_rectangle(
            (642 + i * 143, 567, 765 + i * 143, 597), 5, "#ff6f30"
        )
        img.text(
            name,
            (642 + i * 143, 765 + i * 143),
    ...
    if (h1 := sum(i.height for i in talent_imgs) + (len(talent_imgs) - 1) * 50) >= (
        ...
        img.text(
            "制作：西北一枝花&惜月", 60, img.height - 90, prop_name_font, "#252525"
        )
    else:
        this_height = h2 + 42
        img.stretch((719, 2217), this_height - 40, "height")
    ...
    if chara.name == "旅行者":
        save_name = (
            f"旅行者{chara.element}"
            if chara.icon.endswith("PlayerGirl")
            else f"旅行者男{chara.element}"
        )
    else:
