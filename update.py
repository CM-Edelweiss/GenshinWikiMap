...
def update_constant():
    ...
    if monster_list := ambr_requests(MONSTER_LIST_API):
        save_json(monster_list["items"], RAW / "monster_list.json")
        print(">>>原魔列表raw更新完成")
    else:
        print(">>>原魔列表raw更新失败")
    time.sleep(2)
    ...
    for avatar_detail in avatar_detail_file:
        ...
        if save_file.exists():
            ...
            avatar_info = {
                ...
                "rank": (
                    5 if avatar_detail["Quality"] == 105 else avatar_detail["Quality"]
                ),
                ...
                "icon": {
                    ...
                    "splash": avatar_detail["Icon"]
                    .replace("Icon", "Img")
                    .replace("UI_", "UI_Gacha_"),
        ...
        if not (
            avatar_icon := RESOURCES / "avatar" / f"{avatar_info['icon']['avatar']}.png"
        ).exists():
            download_from_ambr(avatar_icon)
        if not (
            avatar_icon_n := RESOURCES / "avatar_new_m" / f"{avatar_info['name']}.png"
        ).exists():
            shutil.copy2(avatar_icon, avatar_icon_n)
        if not (
            avatar_side_icon := RESOURCES
            / "avatar_side"
            / f"{avatar_info['icon']['side']}.png"
        ).exists():
            try:
                download_img(
                    f"https://upload-bbs.mihoyo.com/game_record/genshin/character_side_icon/{avatar_info['icon']['side']}.png",
                    avatar_side_icon,
                )
            except Exception:
                try:
                    download_img(
                        f"https://enka.network/ui/{avatar_info['icon']['side']}.png",
                        avatar_side_icon,
                    )
                except Exception:
                    pass
        if not (
            avatar_splash_icon := RESOURCES
            / "splash"
            / f"{avatar_info['icon']['splash']}.png"
        ).exists():
        ...
        avatar_info["property"] = [
            ...
            },
            {
                ...
                "promote": [
                    p["addProps"]["FIGHT_PROP_BASE_ATTACK"]
                    for p in ambr_data["upgrade"]["promote"][1:]
                ],
            },
            {
                ...
                "promote": [
                    p["addProps"]["FIGHT_PROP_BASE_DEFENSE"]
                    for p in ambr_data["upgrade"]["promote"][1:]
                ],
            },
            {
                "name": list(ambr_data["upgrade"]["promote"][-1]["addProps"].keys())[
                    -1
                ],
                "promote": [
        ...
        c3_name = get_describe_name(
            avatar_detail["SkillDepot"]["Talents"][2]["Description"]
        )
        c5_name = get_describe_name(
            avatar_detail["SkillDepot"]["Talents"][4]["Description"]
        )
...
def update_material():
    ...
    avatar_infos = [
        ...
        for avatar_id in load_json(RAW / "avatar_list.json")
    ]
    avatar_materials: dict[str, list[int]] = {
        avatar.name: avatar.get_material_ids() for avatar in avatar_infos
    }
    for material in material_dict.values():
        if (MATERIAL_RAW / f'{material["id"]}.json').exists():
            ...
            material_info = (
                ambr_requests(MATERIAL_INFO_API.format(material["id"])) or {}
            )
        ...
        if material_info.get("source"):
            material["获取途径"] = [
                p["name"] for p in material_info["source"]
            ]  # type: ignore
        material["适用角色"] = [
            name for name, mats in avatar_materials.items() if material["id"] in mats
        ]
...
def update_weapon():
    ...
    for weapon_id, weapon_data in weapon_list.items():
        if not weapon_data["name"]:
            continue
        if not (
            weapon_icon := RESOURCES / "weapon" / f"{weapon_data['icon']}.png"
        ).exists():
        ...
        weapon_type = (
            weapon_data["type"]
            if weapon_data["type"] in {"单手剑", "双手剑", "长柄武器", "法器", "弓"}
            else prop_map[weapon_data["type"]]
        )
...
def update_artifact():
    ...
    for suit_id, suit_data in suit_list.items():
        ...
        for part_type, artifact in suit_info["suit"].items():
            ...
            if not (
                icon_file := RESOURCES / "artifact" / f"{artifact_icon}.png"
            ).exists():
